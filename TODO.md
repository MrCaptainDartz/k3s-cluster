...
## 1. Supervision des Backups — alertes sur échec

**Pourquoi en premier :** trou béant actuel — ni les snapshots etcd (locaux, 6h) ni les CronJobs `volume-snapshotter` (PVC Prometheus/Grafana/Loki, quotidien 2h) n'alertent en cas d'échec ; un backup silencieusement cassé ne se découvre qu'au restore. Coût : un après-midi.
...
## 2. Fiabilité des Workloads critiques — PDB, requests, PriorityClass
...
## 3. Automatisation des Dépendances (Renovate)
...
## 4. Alertes GitOps via les métriques ArgoCD (PAS argocd-notifications)
...
## 5. Pre-commit hooks sur le repo GitOps
...
## 6. Vrai certificat Let's Encrypt pour l'Ingress ArgoCD
...
## 7. SSO — Authentik (+ CloudNativePG en prérequis)
...
## 10. Kyverno — validatif minimal uniquement
...
- [ ] PDB + requests sur les pods Kyverno eux-mêmes (cf. §2).
- [ ] NetworkPolicy dédiée.

## 11. Gateway API (opportuniste)
...
**Pourquoi :** l'architecture rend déjà le rebuild quasi automatique (Terraform → ansible-services → ansible-k3s → GitOps → ESO remonte les secrets) — autant documenter et éprouver l'ordre exact une fois, sinon ça ne vaut rien le jour J. Complète `PLAN_MIGRATION.md` (scénario inverse).
...
## 14. Falco (reliquat — dernière priorité)
