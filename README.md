...
- Runs **Traefik v3** (HTTPS reverse proxy with 90-day ECDSA P-384 certificates auto-renewed by OpenBao PKI), **Forgejo** (local Git server), and **OpenBao 2.6** (open-source secret manager with Raft storage & internal PKI) using **Podman Rootless** under an unprivileged user without sudo rights (`services`).
